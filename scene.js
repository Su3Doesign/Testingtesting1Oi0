...
function buildSkillHub(){
  $('skillhub-right').innerHTML = WORKS.map(w => `
    <div class="sh-row" data-cursor="hover">
      <div class="sh-num">N° ${w.no}</div>
      <div class="sh-title-row">${w.title}</div>
      <div class="sh-role">${w.role}</div>
      <div class="sh-year">${w.year}</div>
      <div class="sh-desc">${w.desc}</div>
    </div>
  `).join('');
}
function buildVault(){
  $('vault-grid').innerHTML = VAULT.map(v => `
    <div class="vault-card" data-cursor="hover">
      <div class="vc-no">N° ${v.no}</div>
